...
export function scoreHeavyHaulConfidence(input: ConfidenceInput): HeavyHaulConfidence {
  ...
  if (!input.linehaulPriced) {
    ...
    add({
      ...
      detail:
        'The line haul is OUR MARKET ESTIMATE, not a rate you supplied. It is anchored on a national flatbed line-haul rate published weekly, checked against 2,636 observed flatbed postings, and adjusted by a distance curve measured from about 6,100 real lane observations — all of which are strong. The trailer multiplier that turns a flatbed rate into a heavy-haul rate is weaker, and it is weaker in different ways at different weights: the step-deck figure is measured on 10,245 postings, the RGN figure is bracketed by two methods that disagree, and the permitted multi-axle figure is corroborated by two independent inferences but has never been observed, because no free index publishes heavy-haul rates and no open marketplace carries the transactions. Enter your own $/mile and this deduction goes away, because your negotiated rate is a real price and this is a band.',
