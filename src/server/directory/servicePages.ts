...
function honestyBanner(mode: DataMode, category: string): string {
  ...
  return `<div class="svc-note svc-note-claim"><b>${esc(
    ...
    category,
  )}. The carriers shown are real FMCSA drayage carriers listed only as a starting point. Run a fleet? <a href="/claim">Claim your profile — free</a> to self-declare it.</div>`;
}
