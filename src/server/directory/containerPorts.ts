/**
 * Major US container ports + inland intermodal hubs, and the nearest-hub
 * derivation for the carrier directory.
 *
 * The directory lets shippers browse carriers BY HUB (which drayage/intermodal
 * carriers sit near which gateway). We seed the top US container gateways plus
 * the inland rail-intermodal metros (Chicago, Minneapolis, Denver, Dallas, …)
 * with lat/lng, then map each carrier's physical ZIP → the nearest hub using the
 * 5-digit ZCTA centroid table (src/calc/zip5Centroids.ts, public-domain Census
 * data).
 *
 * Codes are UN/LOCODEs where one exists; USCHI is used for the Chicago inland
 * intermodal complex and `INL*` codes for the other inland rail metros.
 * Coordinates are the marine terminal / rail-hub area, rounded to 4 decimals.
 *
 * Every hub carries a `kind` — 'seaport' (marine container terminals) or
 * 'inland-hub' (a rail ramp with NO marine berth) — so page copy never calls an
 * inland ramp a "port" or talks about its "marine terminals". The URL space is
 * unchanged: every hub, whatever its kind, still lives at /directory/port/:code
 * (61 canonical, sitemapped URLs). Only the WORDING is kind-aware.
 *
...
/**
 * What a hub physically IS. 'seaport' = a coastal/river container gateway with
 * marine terminals; 'inland-hub' = an inland Class-I rail intermodal ramp (no
 * marine berth — containers arrive by rail, not ship).
 */
export type HubKind = 'seaport' | 'inland-hub';

export interface ContainerPort {
  /** UN/LOCODE (or USCHI / INL* for an inland rail hub). */
  code: string;
  name: string;
  city: string;
  state: string;
  /** Domicile country of the hub: 'US' (default) or 'CA'. */
  country: string;
  /** Seaport vs inland rail hub — drives the page nouns (see hubNoun). */
  kind: HubKind;
  lat: number;
  lng: number;
}
...
/**
 * Noun for a hub in running copy: a seaport is a "port"; an inland rail ramp is
 * an "intermodal hub" (it has no berth, so "port" is simply wrong). An absent
 * kind (legacy callers / test fixtures) reads as a seaport, the historic default.
 */
export function hubNoun(kind?: HubKind | null): 'port' | 'intermodal hub' {
  return kind === 'inland-hub' ? 'intermodal hub' : 'port';
}

/**
 * What the hub is a gateway TO, for the "carriers whose nearest … is X" intro:
 * a seaport is a "US container gateway" (or "Canadian container gateway"); an
 * inland ramp is an "inland rail ramp" on either side of the border.
 */
export function hubGatewayLabel(kind?: HubKind | null, country?: string | null): string {
  if (kind === 'inland-hub') return 'inland rail ramp';
  return country === 'CA' ? 'Canadian container gateway' : 'US container gateway';
}

/** Top US container gateways + the Chicago inland intermodal hub. */
export const CONTAINER_PORTS: readonly ContainerPort[] = [
  { code: 'USLAX', name: 'Port of Los Angeles', city: 'Los Angeles', state: 'CA', country: 'US', kind: 'seaport', lat: 33.7395, lng: -118.2597 },
  { code: 'USLGB', name: 'Port of Long Beach', city: 'Long Beach', state: 'CA', country: 'US', kind: 'seaport', lat: 33.755, lng: -118.216 },
  ...
  { code: 'USNYC', name: 'Port of New York & New Jersey', city: 'Newark', state: 'NJ', country: 'US', kind: 'seaport', lat: 40.6816, lng: -74.1505 },
  // FIX: was 29.7264,-95.227 (Turning Basin) → Barbours Cut Container Terminal.
  { code: 'USHOU', name: 'Port of Houston', city: 'Houston', state: 'TX', country: 'US', kind: 'seaport', lat: 29.6819, lng: -94.9983 },
  // FIX: was 32.0835,-81.0998 (downtown Savannah) → Garden City Terminal.
  { code: 'USSAV', name: 'Port of Savannah', city: 'Savannah', state: 'GA', country: 'US', kind: 'seaport', lat: 32.121, lng: -81.135 },
  { code: 'USSEA', name: 'Northwest Seaport Alliance (Seattle/Tacoma)', city: 'Seattle', state: 'WA', country: 'US', kind: 'seaport', lat: 47.5952, lng: -122.3316 },
  // FIX: was 32.81,-79.92 (downtown Charleston) → Wando Welch Terminal.
  { code: 'USCHS', name: 'Port of Charleston', city: 'Charleston', state: 'SC', country: 'US', kind: 'seaport', lat: 32.848, lng: -79.873 },
  { code: 'USORF', name: 'Port of Virginia (Norfolk)', city: 'Norfolk', state: 'VA', country: 'US', kind: 'seaport', lat: 36.873, lng: -76.33 },
  // Chicago is a rail-ramp complex (LPC Elwood / Global IV / Corwith / …) with no
  // marine berth — it sits in this legacy list for the US facet set, but it is
  // an inland hub, never a "port".
  { code: 'USCHI', name: 'Chicago Intermodal Hub', city: 'Chicago', state: 'IL', country: 'US', kind: 'inland-hub', lat: 41.837, lng: -87.67 },
  // FIX: was 39.24,-76.57 → Seagirt Marine Terminal.
  { code: 'USBAL', name: 'Port of Baltimore', city: 'Baltimore', state: 'MD', country: 'US', kind: 'seaport', lat: 39.2592, lng: -76.5436 },
  { code: 'USMIA', name: 'Port of Miami (PortMiami)', city: 'Miami', state: 'FL', country: 'US', kind: 'seaport', lat: 25.778, lng: -80.174 },
];
...
export const CA_CONTAINER_PORTS: readonly ContainerPort[] = [
  { code: 'CAVAN', name: 'Port of Vancouver', city: 'Vancouver', state: 'BC', country: 'CA', kind: 'seaport', lat: 49.2889, lng: -123.1139 },
  { code: 'CAPRR', name: 'Port of Prince Rupert', city: 'Prince Rupert', state: 'BC', country: 'CA', kind: 'seaport', lat: 54.3167, lng: -130.3209 },
  { code: 'CAMTR', name: 'Port of Montreal', city: 'Montreal', state: 'QC', country: 'CA', kind: 'seaport', lat: 45.5586, lng: -73.5175 },
  { code: 'CAHAL', name: 'Port of Halifax', city: 'Halifax', state: 'NS', country: 'CA', kind: 'seaport', lat: 44.6488, lng: -63.5752 },
  { code: 'CASJB', name: 'Port of Saint John', city: 'Saint John', state: 'NB', country: 'CA', kind: 'seaport', lat: 45.2652, lng: -66.0763 },
];
 ...
 * (terminals.ts). Shaped as ContainerPort — the terminal `type` becomes `kind`
 * ('rail' → 'inland-hub', 'seaport' → 'seaport'). Deduped by code — a code
 * already present in CONTAINER_PORTS / CA_CONTAINER_PORTS wins, so the existing
 * seaport coordinates stay authoritative.
 */
export const ALL_HUBS: readonly ContainerPort[] = (() => {
  ...
  for (const t of INTERMODAL_TERMINALS) {
    ...
    seen.set(t.code, {
      ...
      state: t.state,
      country: t.country,
      kind: t.type === 'rail' ? 'inland-hub' : 'seaport',
      lat: t.lat,
      lng: t.lng,
    });
  }
...
export interface PortGroup {
  ...
  state: string;
  country: string;
  /** Seaport vs inland rail hub (a merged group takes its anchor member's kind). */
  kind: HubKind;
