...
async function addr(p,sel,zip){
  const el=await p.$(sel); await el.click(); await el.fill(''); await el.type(zip,{delay:60});
...
async function clickQuote(p){
  ...
  const qb=qh.asElement(); if(qb) await qb.click({force:true});
}
...
for(const [vn,vp] of Object.entries(VPS)){
  ...
  for(const mode of MODES){
    ...
    rec.pickup=await addr(p,'#qf-pickup-zip','90802');
    rec.delivery=await addr(p,'#qf-delivery-zip','85001');
    await p.waitForTimeout(400);
    await clickQuote(p); await p.waitForTimeout(1500);
    rec.err1=await p.$eval('#qf-error',e=>e.offsetParent!==null?e.innerText.trim().slice(0,120):'').catch(()=>'');
    // second click to pass soft-confirm if needed
    if(rec.err1){ await clickQuote(p); await p.waitForTimeout(2500); }
    rec.resultVisible=await p.$eval('#qf-result',e=>e.offsetParent!==null).catch(()=>false);
    rec.resultText=await p.$eval('#qf-result',e=>e.offsetParent!==null?e.innerText.replace(/\s+/g,' ').slice(0,350):'(hidden)').catch(()=>'(none)');
    rec.err2=await p.$eval('#qf-error',e=>e.offsetParent!==null?e.innerText.trim().slice(0,120):'').catch(()=>'');
    await p.screenshot({path:`${OUT}/otr-${tag}-PRICE.png`,fullPage:true});
    log.push(rec);
  }
  await p.close();
}
