...
const box=await p.locator('.qf-hero-pills').boundingBox();
await p.screenshot({path:path.join(OUT,'pills-pop.png'),clip:{x:Math.max(0,box.x-24),y:Math.max(0,box.y-24),width:box.width+80,height:box.height+60}});
